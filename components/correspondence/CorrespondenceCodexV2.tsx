...
export default function CorrespondenceCodexV2() {
  ...
  return <div className={styles.shell}>
    ...
    {index && <div className={styles.workspace}>
      ...
      <section className={styles.detailArea} id="codex-detail" aria-label="Selected correspondence">
        ...
        {detail && <>
          ...
          {detail.related.length > 0 && <section className={styles.relatedBlock}><h3>Follow the thread</h3><p>Shared correspondences offer paths to explore, not proof of historical influence.</p>{detail.related.map((link) => <button key={link.id} type="button" onClick={() => choose(link.id)}>{link.label} <span>↗</span><small>{link.sourceSystemLabel} · row {link.sourceRow}{link.sharedValues.length ? ` · ${link.sharedValues.slice(0, 2).join(' / ')}` : ''}</small></button>)}</section>}
        </>}
    ...
    <section className={styles.classicPanel} aria-label="Classic catalog entries not in matrix">
      <div className={styles.classicIntro}><div><p className={styles.eyebrow}>THE ORIGINAL ATLAS · CLASSIC EDITION</p><h2>There’s more to the story.</h2><p>Browse profiles from the original edition—including deities, stones, and other subjects you won’t find as separate entries here. The original filters, summaries, overviews, and detailed pages are still there whenever you want to go deeper.</p></div><button type="button" aria-expanded={showClassic} aria-controls="codex-classic-extras" onClick={() => setShowClassic((open) => !open)}>{showClassic ? 'Hide Classic-only entries' : 'Browse Classic-only entries ↗'}</button></div>
      {showClassic && <div id="codex-classic-extras"><label htmlFor="codex-classic-search" className={styles.label}>Search the Classic-only catalog</label><input id="codex-classic-search" className={styles.search} value={classicQuery} onChange={(event) => { setClassicQuery(event.target.value); setClassicVisible(24) }} placeholder="Try Fire Agate, Hephaestus, runes…" autoComplete="off" />
        ...
        {classicEntries && <><p className={styles.smallNote}>{classicResults.length} matching Classic-only entries · {classicEntries.length} profiles from the original edition not listed separately in the new Codex. Related ideas may still appear elsewhere.</p><div className={styles.classicResults}>{classicResults.slice(0, classicVisible).map((entry) => <a key={entry.slug} href={classicEntryHref(entry.slug)}><strong>{entry.name}</strong><span>{entry.kind} · Classic profile ↗</span></a>)}</div>{classicVisible < classicResults.length && <button type="button" className={styles.more} onClick={() => setClassicVisible((count) => count + 24)}>Show more · {classicResults.length - classicVisible} remain</button>}</>}
      </div>}
    </section>
    <section className={styles.atlasCta} aria-labelledby="atlas-invitation">
      <div className={styles.atlasCtaCopy}>
        <p className={styles.eyebrow}>BEYOND THE SCREEN · THE CORRESPONDENCE CODEX</p>
        <h2 id="atlas-invitation">Seven volumes. One living atlas.</h2>
        <p>Follow the symbols across the seven-volume Correspondence Codex, then bring the map into your own practice with its 20-page printable Companion. A doorway for the curious; a field guide for the devoted.</p>
        <div className={styles.atlasCtaActions}>
          <a href="https://codexoracle.org/" target="_blank" rel="noopener noreferrer">Explore the seven-volume atlas <span aria-hidden="true">↗</span></a>
          <a href="https://vaultofarcana.com/shop/etsy-4513179705">Discover the Companion <span aria-hidden="true">↗</span></a>
        </div>
      </div>
      <svg className={styles.atlasMark} viewBox="0 0 240 240" fill="none" aria-hidden="true"><circle cx="120" cy="120" r="91"/><circle cx="120" cy="120" r="59"/><path d="M120 23 205 168 35 168 120 23ZM120 213 35 72h170L120 213Z"/><circle cx="120" cy="120" r="7" fill="currentColor" stroke="none"/></svg>
    </section>
    <footer className={styles.footer}>Explore the correspondences, follow a thread into the original Classic edition, or bring a question to the Oracle. <a href="/oracle">Ask the Oracle ↗</a> <a href="/correspondence-engine/classic">Open the Classic edition ↗</a></footer>
  </div>
}
