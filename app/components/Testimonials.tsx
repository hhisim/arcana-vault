      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="glass-card p-8 flex flex-col transition-all duration-300 hover:shadow-[0_0_20px_rgba(147,112,219,0.2)]"
            >
              <p className="text-[#E8E0F0] italic leading-relaxed mb-6 flex-1">
                "{t.quote}"
              </p>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-[#7B5EA7] flex items-center justify-center text-[#E8E0F0] font-cinzel font-bold">
                  {t.author}
                </div>
                <div>
                  <div className="text-[#E8E0F0] font-medium">{t.author}</div>
                  <div className="text-[#9B93AB] text-sm">{t.role}</div>
                </div>
              </div>
            </div>
          ))}
